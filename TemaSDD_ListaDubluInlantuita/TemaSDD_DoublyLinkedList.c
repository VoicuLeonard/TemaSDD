...
void deleteCarByYear(DLList* list, int year) {
	// Verific?m dac? lista nu este goal?
	if (list->start) {
		Node* current = list->start;

		// Parcurgem lista
		while (current) {
			// Dac? g?sim un nod care corespunde condi?iei
			if (current->car.year==year) {
				Node* toBeDeleted = current;

				// Dac? nodul de ?ters este primul din list?
				if (current == list->start) {
					list->start = current->next;
					if (list->start) {
						list->start->prev = NULL; // Actualiz?m pointerul `prev` al noului prim nod
					}
					else {
						list->end = NULL; // Dac? lista devine goal?, actualiz?m ?i `end`
					}
				}
				// Dac? nodul de ?ters este ultimul din list?
				else if (current == list->end) {
					list->end = current->prev;
					if (list->end) {
						list->end->next = NULL; // Actualiz?m pointerul `next` al noului ultim nod
					}
					else {
						list->start = NULL; // Dac? lista devine goal?, actualiz?m ?i `start`
					}
				}
				// Dac? nodul de ?ters este undeva la mijloc
				else {
					current->prev->next = current->next; // Leg?m nodul anterior de nodul urm?tor
					current->next->prev = current->prev; // Leg?m nodul urm?tor de nodul anterior
				}

				// ?tergem nodul
				current = current->next; // Avans?m la urm?torul nod înainte de a ?terge
				free(toBeDeleted);
			}
			else {
				// Dac? nu g?sim un nod care corespunde condi?iei, trecem la urm?torul nod
...
int main() {
	...
	Car car2 = { 2006, "Skoda Octavia Tour",1.9 };
	insertAtBeginning(&list, car2);
	printf("\n----------------------------\n");
	parseBtoE(list);
